'use client'
import { type Species, type Enclosure, useOrgEnclosures, useOrgEnclosuresForSpecies} from '@/lib/react-query/queries'
...
export default function SpeciesRow({species} : {species: Species}){
  ...
  const handleEnclosureClick = (enclosure: Enclosure) => {
	...
	return (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="shrink-0 text-xs">
                      {useEnclosures?.length }{' '}
                      {useEnclosures?.length === 1 ? 'enclosure' : 'enclosures'}
                    </Badge>
                  </div>
      ...
      {selectedEnclosure && <EnclosureDialog
